...
const AdminEditOrder = () => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    setCustomer({
                        ...
                        companyName: o.shippingInfo?.companyName || ""
                    });
                    setBillingAddress({
                        addressLine1: o.billingInfo?.address || o.shippingInfo?.address || "",
                        addressLine2: "",
                        city: o.billingInfo?.city || o.shippingInfo?.city || "",
                        postalCode: o.billingInfo?.pincode || o.shippingInfo?.pincode || "",
                        state: o.billingInfo?.state || o.shippingInfo?.state || "Gujarat",
                        country: o.billingInfo?.country || o.shippingInfo?.country || "India"
                    });
                    ...
                    const formattedItems = o.items.map(i => ({
                        ...
                        unitPrice: i.price,
                        quantity: i.quantity,
                        weightKg: i.weightKg || 1,
                        selectedVariantInfo: i.variant || ""
                    }));
                    setItems(formattedItems);
                    
                    let pMethod = "ONLINE";
                    if (o.paymentMethod === "COD") pMethod = "Cash on Delivery";
                    else if (o.paymentMethod === "STORE_PICKUP") pMethod = "STORE PICK-UP (THIS IS NOT CASH ON DELIVERY OPTION)";
                    else if (o.paymentMethod === "MANUAL TRANSFER") pMethod = "MANUAL TRANSFER";
                    setPaymentMethod(pMethod);
                    
                    setIsTaxExempt(o.isTaxExempt || false);
                    setCreateTaxInvoice(o.generateTaxInvoice !== false);
                    setDiscountValue(o.discount ? String(o.discount) : "");
                    setDiscountType(o.discountType === "PERCENTAGE" ? "%" : "FLAT");
                    setShippingCostOverride(o.shippingAmount ? String(o.shippingAmount) : "0");
                    setShippingMethod(o.selectedShippingProvider || o.courierPartner || "");
                }
            } catch (err) {
    ...
    if (loadingOrder) return <div className="p-10 text-center">Loading Order Details...</div>;

    return (
        <div className="min-h-screen bg-[#f4f6f8] p-4 sm:p-6 lg:p-8 font-sans text-[#202223] relative">
            <div className="max-w-6xl mx-auto mb-6">
                <h2 className="text-[24px] font-bold text-[#1a1a1a]">Edit Order #{id.substring(id.length - 6)}</h2>
            </div>
