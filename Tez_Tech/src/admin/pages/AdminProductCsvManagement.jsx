...
const AdminProductCsvManagement = () => {
  ...
  const handleClearCatalog = async () => {
    const confirmText = window.prompt("🚨 WARNING: This will permanently delete ALL products, categories, and CSV import history from the database.\n\nTo confirm, please type 'DELETE ALL':");
    if (confirmText !== "DELETE ALL") {
      alert("Clear Catalog action cancelled.");
      return;
    }

    try {
      setLoading(true);
      const res = await axios.delete(`${getApiUrl()}/products/admin-clear-all`, { headers });
      if (res.data.success) {
      ...
      console.error(err);
      alert("❌ Clear Catalog failed: " + (err.response?.data?.message || err.message));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col justify-between gap-4 mb-8 xl:flex-row xl:items-center">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button 
            ...
            disabled={loading}
            className="flex items-center gap-2 px-5 py-2.5 font-semibold text-white transition-all duration-300 shadow-lg bg-gradient-to-r from-red-500 to-rose-600 rounded-xl hover:shadow-red-500/40 hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed"
          >
