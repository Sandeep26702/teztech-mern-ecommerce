        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
          ...
          <div className="lg:col-span-8">
            <div className="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-2xl">
              {cartItems.map((item) => {
                // 🔥 UNIQUE KEY FIX: Product ID use karna sabse safe hai
                const itemKey = item.productId?._id || item._id;
                const itemImage = item.productId?.images?.[0]?.url || item.productId?.images?.[0] || item.image;
                const itemName = item.productId?.name || item.name;
                const itemPrice = item.productId?.price || item.price;
                ...
                return (
                  <div key={itemKey} className="flex flex-col gap-4 p-4 border-b border-gray-100 sm:flex-row sm:items-center last:border-0 hover:bg-gray-50/50">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="mt-1 text-sm font-medium text-gray-500">Price: ₹{itemPrice}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex items-center bg-white border border-gray-200 rounded-lg shadow-sm">
                        <button 
                          onClick={() => updateCartQuantity(itemKey, Math.max(1, item.quantity - 1))}
                    ...
                    <div className="text-right sm:w-24">
                      <p className="text-lg font-black text-gray-900">₹{itemPrice * item.quantity}</p>
                    </div>
                  </div>
                );
              })}
